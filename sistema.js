// Utility Functions
    ...
    class BirthdayExperience {
        ...
        loop() {
            ...
            if (this.state === 'playing' && this.audio.isFinished()) {
                this.state = 'finished';
                this.effects.clear();
    ...
    // Lyric Manager Class --- SECCIÓN MODIFICADA ---
    class LyricManager {
        constructor() {
            this.container = document.getElementById('lyrics-container');
            // La estructura base de la canción está en inglés.
            this.songStructure = [
                ...
                { time: 33, type: 'chorus', line: "'Cause you're a sky, 'cause you're a sky full of stars" },
                { time: 38, type: 'verse', line: "'Cause you light up the path" },
                { time: 43, type: 'verse', line: "I don't care, go on and tear me apart" },
                { time: 48, type: 'verse', line: "I don't care if you do, ooh-ooh, ooh" },
                { time: 53, type: 'chorus', line: "'Cause in a sky, 'cause in a sky full of stars" },
                { time: 58, type: 'chorus', line: "I think I see you" },
                { time: 63, type: 'instrumental', line: '' },
                { time: 83, type: 'chorus', line: "'Cause you're a sky, 'cause you're a sky full of stars" },
                { time: 88, type: 'verse', line: "I wanna die in your arms, oh, oh-oh" },
                { time: 93, type: 'verse', line: "'Cause you get lighter the more it gets dark" },
                { time: 98, type: 'verse', line: "I'm gonna give you my heart, ooh-ooh, ooh" },
                { time: 103, type: 'chorus', line: "And I don't care, go on and tear me apart" },
                { time: 108, type: 'chorus', line: "And I don't care if you do, ooh-ooh, ooh" },
                { time: 113, type: 'chorus', line: "'Cause in a sky, 'cause in a sky full of stars" },
                { time: 118, type: 'chorus', line: "I think I saw you" },
                { time: 123, type: 'instrumental', line: '' },
                { time: 165, type: 'outro', line: "You're such a heavenly view" },
                { time: 175, type: 'outro', line: "You're such a heavenly view" },
                { time: 185, type: 'outro', line: "Happy Birthday!" }
            ];
            // Diccionario de traducciones solo para el español.
            this.lyricsEs = {
        ...
        // Lógica de actualización refactorizada para mayor claridad y seguridad
        update(segment) {
            const originalLine = segment.line; // Variable para la línea base (inglés)
            let lineToShow = originalLine;      // Variable para la línea que se mostrará al final

            // Comprueba si el idioma es español
            if (this.currentLang === 'es') {
                const translatedLine = this.lyricsEs[originalLine]; // Intenta traducir
                // Si se encontró una traducción, úsala
                if (translatedLine) {
                    lineToShow = translatedLine;
                }
            }
            
            // Muestra la línea final (ya sea en inglés o la traducción en español)
            if (lineToShow) {
                this.container.innerHTML = `<p class="lyric-line">${lineToShow}</p>`;
                const lyricElement = this.container.querySelector('.lyric-line');
                void lyricElement.offsetWidth; // Force reflow
                lyricElement.classList.add('visible');
            } else {
                this.container.innerHTML = ''; // Limpia si la línea está vacía (instrumental)
            }
        }
    }
    ...
    class AudioManager {
        ...
        setVolume(normalizedVolume) {
            if (this.volumeNode) {
                const safeVolume = Math.max(0.001, normalizedVolume);
                this.volumeNode.volume.value = 20 * Math.log10(safeVolume);
            }
        }
    ...
    class EffectsManager {
        ...
        createHeart() {
            ...
            heart.style.setProperty('--sway', `${Utils.rand(-50, 50)}px`);
